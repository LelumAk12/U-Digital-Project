  ...
  return <section className="w-full bg-white py-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-4 mb-8 overflow-x-auto pb-2">
          {tabs.map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-2 rounded-full whitespace-nowrap ${activeTab === tab ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
              {tab}
            </button>)}
        </div>
        ...
        <div className="text-center mt-12">
          <button className="bg-teal-600 text-white px-8 py-3 rounded-full hover:bg-teal-700 font-medium">
            View All Projects
          </button>
        </div>
      </div>
